...
TEST01=$(cat <<EOL
Tekst1 koji 77! treba - sifrovati.
8
3
EOL
)
...
OUTP01=$(cat <<EOL
Unesite tekst: Tekst1 koji 77! treba - sifrovati.
Unesite bazu: 8
Unesite kod: 3
Sifrovan tekst: Whnvw1 nrml 77! wuhed - vliurydwl.
Broj sifrovanih karaktera je: 27
EOL
)
...
Unesite kod: 58
Sifrovan tekst:  #:-)-{--<
Broj sifrovanih karaktera je: 0
EOL
)

TEST03=$(cat <<EOL
Nevalidan karakter u bazi
7a6
3
EOL
)
...
OUTP03=$(cat <<EOL
Unesite tekst: Nevalidan karakter u bazi
Unesite bazu: 7a6
EOL
)
...
OUTP05=$(cat <<EOL
Unesite tekst: Kod veci od broja karaktera
Unesite bazu: 2
Unesite kod: 79
Sifrovan tekst: Lpe wfdj pe cspkb lbsblufsb
Broj sifrovanih karaktera je: 10111
EOL
)
...
Prekoracenje kod!
2
878648573840583738475637
EOL
)

OUTP06=$(cat <<EOL
Unesite tekst: Prekoracenje kod!
Unesite bazu: 2
Unesite kod: 878648573840583738475637
EOL
)
...
if [ "$1" != "" ] && [ -f "$1" ]; then
    ...
    gcc -g -m32 -o zad $zfiles 1>$OUT1 2>&1
    if [ $? -ne 0 ]; then
