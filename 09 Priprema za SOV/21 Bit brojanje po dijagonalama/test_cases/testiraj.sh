...
TEST01=$(cat <<EOL
0xff 0xa4 0xb2 0x31 0x78 0x45 0x34 0x12
EOL
)

OUTP01=$(cat <<EOL
Unesite 8 elemenata niza: 0xff 0xa4 0xb2 0x31 0x78 0x45 0x34 0x12
...
OUTP02=$(cat <<EOL
Unesite 8 elemenata niza: 0x01 0x02 0x04 0x08 0x10 0x20 0x40 0x80
PARAN broj jedinica.
EOL
)
...
OUTP03=$(cat <<EOL
Unesite 8 elemenata niza: 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00
PARAN broj jedinica.
EOL
)
...
if [ "$1" != "" ] && [ -f "$1" ]; then
    ...
    gcc -g -m32 -DTESTIRAJ -o zad $GLAVNI $zfiles 1>$OUT1 2>&1
    if [ $? -ne 0 ]; then
        echoq -e "\e[01;31m\n\nGreška u kompajliranju!\e[00m\n"
        if [ $QUIET -eq 0 ]; then
            cat $OUT1
